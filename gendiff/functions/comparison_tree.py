def compare_dicts(dict1, dict2):
    ...
    for key in keys:
        if key in dict1 and key in dict2 and dict1[key] == dict2[key]:
            dict_diff[key] = {'value': dict1[key], 'status': 'same'}
        if key not in dict1 and key in dict2:
            dict_diff[key] = {'value': dict2[key], 'status': 'added'}
        if key in dict1 and key not in dict2:
            dict_diff[key] = {'value': dict1[key], 'status': 'removed'}
        if key in dict1 and key in dict2 and dict1[key] != dict2[key] and \
                isinstance(dict1[key], dict) and isinstance(dict2[key], dict):
            dict_diff[key] = {'value': compare_dicts(dict1[key], dict2[key]),
                              'status': 'updated_dict'}
        if key in dict1 and key in dict2 and dict1[key] != dict2[key] and not \
                (isinstance(dict1[key], dict) and isinstance(dict2[key], dict)):
            dict_diff[key] = {'old_value': dict1[key], 'new_value': dict2[key], 'status': 'updated'}
    return dict_diff